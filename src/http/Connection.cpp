...
{
	if (_pendingResponses.empty()) return OK;

	PendingResponse &item = _pendingResponses.front();
	if (!item.res) return ERROR;
	...
	if (w > 0) {
		...
		if (cur->start >= response.size()) {
			...
			if (close_after) return CLOSED;

			if (!_pendingResponses.empty()) return WANT_WRITE;

			_status = READING_HEADERS;

			Connection::e_result pr =
				_processInput();// try to parse / enqueue immediately so we don't wait for another EPOLLIN.
			if (pr == WANT_WRITE) return WANT_WRITE;
			return OK;
		}
		return WANT_WRITE;
	}
	...
	if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK) return WANT_WRITE;

	if (errno == EPIPE || errno == ECONNRESET) return CLOSED;
...
bool Connection::_shouldKeepAlive(const HttpRequest &req) const
{
	...
	if (proto.find("HTTP/1.1") == 0) return conn != "close";
...
Connection::e_result Connection::_processInput()
{
	...
	while (true) {
		...
		if (!_tryExtractOneRequest()) break;
		if (_pendingResponses.size() > before) queued_any = true;
	}
...
Connection::e_result Connection::_handleReadable()
{
	...
	if (_peerClosedInput && _pendingResponses.empty()) return CLOSED;
...
HttpResponse *Connection::_prepareResponse()
{
	...
	try {
		if (cgi != NULL) {
			if (_parent == NULL) { throw new TCPServer::GenericException(); }
...
void Connection::_prepareResponse_get(HttpResponse *res) const
{
	...
	if (_req->headers["range"].empty()) res->headers["accept-ranges"] = "bytes";
...
void Connection::_prepareResponse_put(HttpResponse *res) const
{
	...
	if (rel_path.empty()) rel_path = "default";
	std::string path = res->location->_root + rel_path;

	if (access(path.c_str(), F_OK) == 0) res->set_response_code(SC_204);
	else
...
void Connection::_parseRange(HttpResponse& res) const
{
	...
	if (end == 0) end = res.body.length() - 1;

	res.headers["content-range"] = "bytes " + ::itoa(start) + "-" + ::itoa(end) + "/" + ::itoa(res.body.length());
	if (end < res.body.length() - 1) res.body.erase(end + 1);
