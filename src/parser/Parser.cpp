...
std::string Parser::getConfigRoot() const { return _config_root; }

unsigned int    Parser::getCurrentLine() const { return _current_line; }

t_token Parser::getCurrentToken() const { return _current; }

t_token Parser::getNextToken() const { return _next; }

std::vector<t_token>    Parser::getTokens() const { return _tokens; }

std::vector<t_token>::iterator  Parser::getCurrentIt() const { return _current_it; }

IBlock* Parser::getCurrentBlock() const { return _current_block; }

HTTP&   Parser::getConfig() { return _config; }

const HTTP&   Parser::getConfig() const { return _config; }

Server& Parser::getLastServer() { return _config.getServers().back(); }

const Server& Parser::getLastServer() const { return _config.getServers().back(); }

Location& Parser::getLastLocation() { return getLastServer().getLocations().back(); }

const Location& Parser::getLastLocation() const { return getLastServer().getLocations().back(); }

CGI& Parser::getLastCGI() { return getLastLocation()._cgi.back(); }

const CGI& Parser::getLastCGI() const { return getLastLocation()._cgi.back(); }

void    Parser::setConfigRoot(std::string config_root) { _config_root = config_root; }

void    Parser::setCurrentToken(t_token current) { _current = current; }

void    Parser::setNextToken(t_token next) { _next = next; }

void    Parser::setCurrentIt(std::vector<t_token>::iterator current_it) { _current_it = current_it; }

void    Parser::setCurrentBlock(IBlock* current) { _current_block = current; }
...
void    Parser::init_key_database()
{
    ...
    fin.open("./resources/keywords.txt", std::ios::in);
